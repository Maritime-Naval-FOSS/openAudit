...
url = "http://OpenAudit.MARITIME.local/openaudit2/index.php/system"
			...
			if objRecordSet.RecordCount > 0 then
				...
				Do Until objRecordSet.EOF
					...
					if os_group = "windows" then
						...
						result.WriteText "		<type>system</type>" & vbcrlf
...
result.WriteText "		<system_type>system</system_type>" & vbcrlf
...
for each objItem in colItems
	if (Instr(objItem.Caption, "vnc") = 0 AND Instr(objItem.Caption, "DameWare") = 0 AND Instr(objItem.Caption, "Innobec SideWindow") = 0 AND Instr(objItem.Caption, "Microsoft SMS Mirror Driver") = 0) then
...
if (skip_software = "n") then
	...
	'		wscript.echo "No subkeys for 64bit (registry)"
	'		reg_node = "n"
	'	end if
	'next
	'on error goto 0
...
if objFSO.FileExists(sScriptPath & "sqlite3.Exe") then
	...
	if db_present then
		...
		do
			...
			key_release = ""
			key_edition = ""
		loop while not rexec.StdOut.AtEndofStream
	else
